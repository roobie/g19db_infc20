...
<div id="tabs" class="left-col">
				...
				buttons: {
					"Create!": function() {
						...
						if ($("input[name='student_type']:checked").val() == "domestic") {
							bValid = bValid && checkLength( ssn, "ssn", 1, 6 );
	...
	<div id="create-course">		
		...
		<div class="form-insert">
		<div id="add-course-message-insert" class="ui-message-box"></div>
		...
		$(function() {
			...
			$( "#add-course-dialog-form" ).dialog({
				...
				buttons: {
					"Add!": function() {
						...
						bValid = bValid && checkLength( ccode, "ccode", 5, 45 );
						bValid = bValid && checkLength( cname, "cname", 3, 45 );
						
						bValid = bValid && checkRegexp( ccode, /^[+]([0-9-])+$/i, "Course code may consist of 0-9" );
						bValid = bValid && checkRegexp( cname, /^[a-zåäö]([0-9a-z_åäö])+$/i, "Course name may consist of a-z + å, ä and ö, 0-9, underscores and must begin with a letter." );


							$.post( "inc/add_course.php", {
								ccode: ccode.val(),
								cname: cname.val(),
			...
			<form class="js-form">
				...
				<br />
				<input type="text" name="ccode" id="ccode" class="text ui-widget-content ui-corner-all" />
				<br />
				<br />
	...
	<div id="search">
		...
		$(function() {
			...
			$( "#search-student-dialog-form" ).dialog({
				autoOpen: false,
				height: 500,
				width: 400,
				modal: true,
			...
			$( "#search-student-button" )
				.button()
				.click(function() {
					$( "#search-student-dialog-form" ).dialog( "open" );
				});
		});
		</script>
			<p class="validateTips">All form fields are required.</p>
